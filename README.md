...
Catatan versi: `umap-learn` bergantung pada `numba` yang mensyaratkan NumPy ≤2.4, karena itu `requirements.txt` mengunci `numpy~=2.4.6` (tetap di bawah 2.5). Pasang dependensi lewat `pip install -r requirements.txt` sebelum menjalankan Phase 1 agar sel UMAP dapat dihitung.
...
Deliverable Phase 1 berupa *clean dataset file* (`data/processed/cleaned_lending_club.csv`, 12 fitur × 2.260.668 baris, sekitar 220 MB) diregenerasi oleh pipeline dan tidak di-commit karena ukurannya. Untuk pemeriksaan tanpa menjalankan pipeline, sampel 100.000 baris berlabel cluster tersedia di `phase5/csv/phase2_clustered_sample.csv`, dan tabel profil lengkapnya ada di Appendix A laporan.
